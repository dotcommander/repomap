...
func TestFileAllDead(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "no exported symbols",
			syms:   []Symbol{{Name: "a", Kind: "function", Exported: false}},
			expect: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tc.expect, fileAllDead(tc.syms))
		})
	}
}
...
func TestDeadExportBudgetCost(t *testing.T) {
	...
	// Build two files with identical symbol counts and signatures.
	// The live file has ImportedBy > 0 so markDeadExports won't mark them Dead.
	// The dead file has ImportedBy == 0 so markDeadExports marks all exported symbols Dead.
	liveSyms := []Symbol{
		{Name: "A", Kind: "function", Signature: "(ctx context.Context) error", Exported: true},
		{Name: "B", Kind: "function", Signature: "(cfg Config) *Server", Exported: true},
	}
	deadSyms := []Symbol{
		{Name: "A", Kind: "function", Signature: "(ctx context.Context) error", Exported: true, Dead: true},
		{Name: "B", Kind: "function", Signature: "(cfg Config) *Server", Exported: true, Dead: true},
	}

	live := RankedFile{FileSymbols: &FileSymbols{Path: "live.go", Symbols: liveSyms}, ImportedBy: 1}
	dead := RankedFile{FileSymbols: &FileSymbols{Path: "dead.go", Symbols: deadSyms}, ImportedBy: 0}

	// Use a budget tight enough that only one file can get level 2.
	// The dead file's halved cost should make it less competitive.
	ranked := BudgetFiles([]RankedFile{live, dead}, 50)
	require.Len(t, ranked, 2)

	// The dead file must have DetailLevel <= the live file's level.
	// With cost halving, the dead file is more likely to be demoted.
	assert.LessOrEqual(t, ranked[1].DetailLevel, ranked[0].DetailLevel,
		"dead-export file should not outrank live file for detail level")
}
