...
func TestReadRepositoryRegularFileLimit(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(root, "bounded.txt"), []byte("123456"), 0o644))

	_, _, err := ReadRepositoryRegularFileLimit(root, "bounded.txt", 5)
	require.ErrorIs(t, err, ErrRepositoryFileTooLarge)

	_, data, err := ReadRepositoryRegularFileLimit(root, "bounded.txt", 6)
	require.NoError(t, err)
