...
func ApplyFixFindings(ctx context.Context, repoRoot string, findings []Finding) (applied, skipped []Finding, err error) {
	...
	for path, group := range byFile {
		_, data, readErr := safefs.ReadRepositoryRegularFile(repoRoot, path)
		if readErr != nil {
			if errors.Is(readErr, safefs.ErrUnsafeRepositoryFile) {
		...
		if dirty {
			...
			if writeErr := safefs.RewriteRepositoryRegularFile(repoRoot, path, newContent); writeErr != nil {
...
func ApplyReviewDecisions(ctx context.Context, repoRoot string, decisions []ReviewDecision, findings []Finding) error {
	...
	for path, edits := range fileEdits {
		_, data, err := safefs.ReadRepositoryRegularFile(repoRoot, path)
		if err != nil {
	...
	for path, edits := range fileEdits {
		...
		if err := safefs.RewriteRepositoryRegularFile(repoRoot, path, []byte(strings.Join(lines, "\n"))); err != nil {
