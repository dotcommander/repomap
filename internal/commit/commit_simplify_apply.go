...
func ApplyCandidates(ctx context.Context, repoRoot string, candidates []Candidate) (applied, skipped []Candidate, err error) {
	for _, c := range candidates {
		...
		_, data, readErr := safefs.ReadRepositoryRegularFile(repoRoot, c.File)
		if readErr != nil {
		...
		if writeErr := safefs.RewriteRepositoryRegularFile(repoRoot, c.File, newContent); writeErr != nil {
